...
<div id="content" class="container-fluid{if isset($MENUBAR)} contentWithMenu{/if}">
...
{/if}
	...
	<div class="row">
		<div class="col-lg-2 col-md-3 col-sm-4 d-none d-sm-block vh-100 sticky-top overflow-auto bg-light border-end">
...
<div class="container-fluid">
	<div class="row py-2">
		...
		<div class="col-lg-3 col-md-10 col-8 lh-lg">
			<div class="browsePath">
				{$SECTION_TITLE}<span class="browsePathSeparator">{$LEVEL_SEPARATOR}</span><h2 class="d-inline fs-6">{$current.TITLE}</h2>
			</div>
		</div>
		<div class="col-lg-4 col-md-6 text-center lh-lg">
			<small class="px-2">{$INFO_CREATION_DATE}</small> &middot; <small class="px-2">{$PHOTO}</small>
		</div>
		<div class="col-lg-4 col-md-6 pe-0">
<div id="imageToolBar">

<div class="actionButtons">
	<a class="pwg-button" role="button" type="button" data-bs-toggle="offcanvas" data-bs-target="#imageInfos_new" aria-controls="imageInfos_new">
		<i class="pwg-icon"><i class="fa-solid fa-circle-info"></i></i>
		<span class="pwg-button-text">{'Photo informations'|@translate}</span>
	</a>
...
function changeImgSrc(url,typeSave,typeMap)
{
	...
	jQuery('#photo-size-switch a.dropdown-item.active').removeClass('active');
	jQuery('#photo-size-switch a.dropdown-item[data-type="' + typeMap + '"]').addClass('active');
	document.cookie = 'picture_deriv='+typeSave+';path={/literal}{$COOKIE_PATH}{literal}';
}
{/literal}{/footer_script}
{strip}

<div id="photo-size-switch" class="dropdown d-inline-block">
  <a title="{'Photo sizes'|@translate}" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false" rel="nofollow">
    <span class="pwg-icon pwg-icon-sizes"></span><span class="pwg-button-text">{'Photo sizes'|@translate}</span>
  </a>
  <ul class="dropdown-menu dropdown-menu-end">
  	<li><h6 class="dropdown-header">{'Photo sizes'|@translate}</h6></li>
  	{foreach from=$current.unique_derivatives item=derivative key=derivative_type}
    <li>
    	<a data-type="{$derivative->get_type()}" class="dropdown-item {if $derivative->get_type() eq $current.selected_derivative->get_type()}active{/if}" {if $derivative->get_type() eq $current.selected_derivative->get_type()}aria-current="true"{/if} href="javascript:changeImgSrc('{$derivative->get_url()|@escape:javascript}','{$derivative_type}','{$derivative->get_type()}')">
    ...
    <li><hr class="dropdown-divider"></li>
		<li><a class="dropdown-item" href="javascript:phpWGOpenWindow('{$U_ORIGINAL}','xxx','scrollbars=yes,toolbar=no,status=no,resizable=yes')" rel="nofollow">{'Original'|@translate}</a></li>
		{/if}
  </ul>
</div>
{/strip}
...
{strip}{if isset($favorite)}
	<a href="{$favorite.U_FAVORITE}" title="{if $favorite.IS_FAVORITE}{'delete this photo from your favorites'|@translate}{else}{'add this photo to your favorites'|@translate}{/if}" class="pwg-state-default pwg-button" rel="nofollow">
		<span class="pwg-icon">{if $favorite.IS_FAVORITE}<i class="fa-solid fa-heart"></i>{else}<i class="fa-regular fa-heart"></i>{/if}</span>
		<!--<span class="pwg-icon pwg-icon-favorite-{if $favorite.IS_FAVORITE}del{else}add{/if}"></span>-->
...
<div class="offcanvas offcanvas-bottom" data-bs-scroll="true" tabindex="-1" id="imageInfos_new" aria-labelledby="offcanvasBottomLabel">
  <div class="offcanvas-header">
    <h5 class="offcanvas-title" id="offcanvasBottomLabel">{'Photo informations'|@translate}</h5>
    <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
  </div>
  <div class="offcanvas-body">
  	<dl id="standard" class="">
		{strip}
	    <div class="row">
				{if $display_info.author and isset($INFO_AUTHOR)}
				<div id="Author" class="col-lg-3 mb-3">
					<dt>{'Author'|@translate}</dt>
					<dd>{$INFO_AUTHOR}</dd>
				</div>
				{/if}
				{if $display_info.created_on and isset($INFO_CREATION_DATE)}
				<div id="datecreate" class="col-lg-3 mb-3">
					<dt>{'Created on'|@translate}</dt>
					<dd>{$INFO_CREATION_DATE}</dd>
				</div>
				{/if}
				{if $display_info.posted_on}
				<div id="datepost" class="col-lg-3 mb-3">
					<dt>{'Posted on'|@translate}</dt>
					<dd>{$INFO_POSTED_DATE}</dd>
				</div>
				{/if}
				{if $display_info.dimensions and isset($INFO_DIMENSIONS)}
				<div id="Dimensions" class="col-lg-3 mb-3">
					<dt>{'Dimensions'|@translate}</dt>
					<dd>{$INFO_DIMENSIONS}</dd>
				</div>
				{/if}
				{if $display_info.file}
				<div id="File" class="col-lg-3 mb-3">
					<dt>{'File'|@translate}</dt>
					<dd>{$INFO_FILE}</dd>
				</div>
				{/if}
				{if $display_info.filesize and isset($INFO_FILESIZE)}
				<div id="Filesize" class="col-lg-3 mb-3">
					<dt>{'Filesize'|@translate}</dt>
					<dd>{$INFO_FILESIZE}</dd>
				</div>
				{/if}
				{if $display_info.tags and isset($related_tags)}
				<div id="Tags" class="col-lg-3 mb-3">
				...
				<div id="Categories" class="col-lg-3 mb-3">
				...
				<div id="Visits" class="col-lg-3 mb-3">
				...
				{if $display_info.rating_score and isset($rate_summary)}
					<div id="Average" class="col-lg-3 mb-3">
				...
				{if isset($rating)}
					<div id="rating" class="col-lg-3 mb-3">
						<dt>
				...
				{if $display_info.privacy_level and isset($available_permission_levels)}
					<div id="Privacy" class="col-lg-3 mb-3">
...
<div id="comments" {if (!isset($comment_add) && ($COMMENT_COUNT == 0))}class="noCommentContent"{else}class="commentContent"{/if}><div id="commentsSwitcher"></div>
	<h3 class="fs-4">{$COMMENT_COUNT|@translate_dec:'%d comment':'%d comments'}</h3>

	<div id="pictureComments">
		{if isset($comment_add)}
		<div id="commentAdd">
			<h4 class="fs-5">{'Add a comment'|@translate}</h4>
			<form method="post" action="{$comment_add.F_ACTION}" id="addComment">
				{if $comment_add.SHOW_AUTHOR}
					...
					<p><input type="text" class="form-control" name="author" id="author" value="{$comment_add.AUTHOR}"></p>
				{/if}
				{if $comment_add.SHOW_EMAIL}
					...
					<p><input type="text" class="form-control" name="email" id="email" value="{$comment_add.EMAIL}"></p>
				{/if}
        ...
        {/if}
				...
				<p><input type="hidden" name="key" value="{$comment_add.KEY}">
					<input type="submit" class="btn btn-primary" value="{'Submit'|@translate}"></p>
			</form>
		</div>
		{/if}
		{if isset($comments)}
